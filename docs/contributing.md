...
We recommend using [Tox](https://tox.wiki/en/latest/index.html) to setup the
development environment. This will create a new virtual environment with all
of the required packages installed and the `proxystore-extensions` package
installed in editable mode.

```bash
$ git clone https://github.com/proxystore/extensions.git
$ cd extensions
...
E.g. the tests for `proxystore_extensions/modA/modB.py` are located in
`tests/modA/modB.py`; however, additional test files can be added
...
$ tox -e py39
# Run a specific test
$ tox -e py39 -- tests/modA.py::test_function_name
```
...
4. Tag the release commit and push (typically this is the commit updating the
   version numbers).
   ```bash
   $ git tag -s v{VERSION} -m "ProxyStore Extensions v{VERSION}"
